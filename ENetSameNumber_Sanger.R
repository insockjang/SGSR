ENetSameNumber_Sanger<-function(pathwayName,dataCombine,ALPHA = 0.5){
  ...
  dataSets<-myData_Sanger(dataCombine,"ActArea")
  ...
  allProcess<-function(kk){  
    filename1 = paste("~/Result_priorIncorporateLasso/",dataCombine,"/Sanger/",pathwayName,"/PriorENet_alpha_",ALPHA,"_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename1)){
      ...
      processer<-function(k){
        ...
        return(list(test.Predictions = P.resp.enet.1,testObservations = filteredResponseDataScaled[foldIndices[[k]]]))
      }  
      require(multicore)
  ...
  require(multicore)
  resultsScale<-mclapply(1:138,function(x)allProcess(x),mc.cores=2)
