restoreSGSR_CCLE<-function(pathwayName,dataCombine){
...
restoreSGSR_Sanger<-function(pathwayName,dataCombine,KK){
  ### DEMO Stepwise grouping Lasso
  require(predictiveModeling)
  require(synapseClient)
  # synapseLogin("in.sock.jang@sagebase.org","tjsDUD@")
  ...
  #   KK<-c(1:24)
  for(kk in KK){
    filename1 = paste("~/Result_priorIncorporateLassoNew_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/PriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
    load(filename1)  
    filename = paste("~/Result_priorIncorporateLassoNew_filterVar02/",dataCombine,"/Sanger/",pathwayName,"/restoredPriorIncorporated_cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){    
