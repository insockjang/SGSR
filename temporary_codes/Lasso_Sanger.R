Lasso_Sanger<-function(dataCombine){
  ...
  processFunction<-function(kk){  
    filename = paste("~/Result_priorIncorporateLasso/",dataCombine,"/Sanger/Lasso/cvDrug_",kk,".Rdata",sep = "")
    if(!file.exists(filename)){
      ...
      resultsScale<-crossValidatePredictiveModel1(filteredFeatureDataScaled, filteredResponseDataScaled, model = myEnetModel1$new(), alpha=1, numFolds=5, nfolds = 5)
